...
function replaceTextInFacebook(correctedText) {
  ...
  if (selection.rangeCount > 0) {
    ...
    while (element && element !== document.body) {
      if (element.getAttribute('role') === 'textbox' || 
          ...
          element.classList.contains('notranslate') && element.getAttribute('style')?.includes('user-select: text')) {
  ...
  if (!activeComposer && document.activeElement) {
  ...
  if (!activeComposer) {
    console.log("No active Facebook composer found");
    return false;
  }
  console.log("Found Facebook composer:", activeComposer);
  try {
    activeComposer.focus();
    if (selection.rangeCount > 0 && !selection.isCollapsed) {
       const success = document.execCommand('insertText', false, correctedText);
       if (success) {
         console.log("Facebook replacement successful with insertText");
         return true;
       }
    } else {
       const success = document.execCommand('insertText', false, correctedText);
       if (success) {
         console.log("Facebook replacement successful with insertText (cursor)");
         return true;
       }
    }
    const dataTransfer = new DataTransfer();
    dataTransfer.setData('text/plain', correctedText);
    ...
    console.error("Facebook replacement failed:", error);
    return false;
  }
}
...
function replaceTextInEditor(correctedText) {
  ...
  if (isFacebook) {
    const result = replaceTextInFacebook(correctedText);
    if (result) {
      console.log("Facebook text replacement successful");
      return true;
    } else {
      console.log("Facebook text replacement failed");
      return false;
    }
  }
  if (isOutlook) {
...
function setupFacebookIntegration() {
  if (!window.location.hostname.includes('facebook.com') && 
      !window.location.hostname.includes('messenger.com')) return;
  const observer = new MutationObserver(() => {
    const composers = document.querySelectorAll('div[role="textbox"], div[contenteditable="true"]');
    composers.forEach(composer => {
      if (!composer.hasAttribute('data-grammar-fix-setup')) {
        composer.setAttribute('data-grammar-fix-setup', 'true');
        console.log("Facebook/Messenger composer detected and set up");
      }
    });
  });
  observer.observe(document.body, { childList: true, subtree: true });
